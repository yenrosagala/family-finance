    ...
    return res.json({
      investments: investments.map((i) => ({
        ...
        gain_loss_pct: Number(i.total_invested) > 0
          ? (Number(i.current_value) - Number(i.total_invested)) / Number(i.total_invested) : 0,
      })),
    });
  } catch (e) {
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      ...
      [req.householdId, String(name).trim(), type, startingValue, startingValue > 0 ? new Date() : null]
    );
  ...
  try {
    ...
    const { rows } = await client.query(
      ...
      [name && String(name).trim() ? String(name).trim() : null,
       type || null, targetVal !== null ? Number(targetVal) : null, targetVal !== null, req.params.id]
    );
