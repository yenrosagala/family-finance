-- =========================================================
-- FAMILY CASH FLOW — AIVEN POSTGRESQL SETUP
-- Apply this in the Aiven SQL/Query console (or psql)
-- against the target database (defaultdb).
-- =========================================================

-- ---------------------------------------------------------
-- USERS (app-level accounts; Aiven has no Supabase auth.users)
-- Backed by the Node backend's auth service.
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- CORE TABLES
-- Order matters: referenced tables are created before
-- transactions (transactions FK-references savings/investments).
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- TRIGGERS — balance sync + saving/investment rollup
...
-- ---------------------------------------------------------
-- (Recommended) SEED default categories for any future household.
-- Optional — skip if you'll seed via the app/backend.
-- ---------------------------------------------------------
-- insert into categories (household_id, name, icon, color, transaction_type, is_default)
-- select '<household-uuid>', c.name, c.icon, c.color, c.type, true
-- from (values
--   ('Salary','briefcase','#10B981','income')
-- ) as c(name, icon, color, type);
